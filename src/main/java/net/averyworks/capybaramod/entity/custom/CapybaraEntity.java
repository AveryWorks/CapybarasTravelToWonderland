...
public class CapybaraEntity extends TamableAnimal implements IAnimatable {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new TemptGoal(this,1.0D, Ingredient.of(Items.MELON_SLICE),false));
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("capybara.animation.walk", true));
            return PlayState.CONTINUE;
        }

        if (this.isSitting()){

            event.getController().setAnimation(new AnimationBuilder().addAnimation("capybara.animation.sitting", true));
            return PlayState.CONTINUE;

        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("capybara.animation.idle", true));
        return PlayState.CONTINUE;
    }
